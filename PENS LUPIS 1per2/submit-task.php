<?php 
    ...
    if(!(isset($_GET['id']))) echo '<script>window.location="tugas.php"</script>';
    if($_SESSION['a_global']->role == 0){
    include 'db.php';
    $id = $_GET['id'];
    $submit_query = mysqli_query($conn,"SELECT * FROM `tb_submit` WHERE tugas_id='".$id."'");
    $submit = mysqli_fetch_object($submit_query);
    $task_id = $id;
    $task_query = mysqli_query($conn,"SELECT * FROM `tb_tugas` WHERE tugas_id='".$task_id."'");
    $task = mysqli_fetch_object($task_query);
    if(mysqli_num_rows($task_query)){
    
    $sub_id = $task->matkul_id;
    $subject_query = mysqli_query($conn,"SELECT * FROM `tb_matkul` WHERE matkul_id='".$sub_id."'");
    $subject = mysqli_fetch_object($subject_query);
?>
                    ...
                    <div class="child-card" style="width: 70%;">
                        <div class="card-content" >
                            <div class="card-header">
                                Mata Kuliah  <strong><?php echo "&nbsp".$subject->matkul_name;?></strong>
                            </div>
                            <div class="card-body">
                                
                                <div class="card-menu-content">
                                    <div class="caption">
                                        Judul Tugas : <strong><?php echo $task->tugas_name;?></strong>
                                        <p>Deskripsi Tugas : <?= $task->tugas_description?></p>
                                        <form action="" method="post" enctype="multipart/form-data">
                                            ...
                                            <input type="file" id="file" placeholder="Lampiran : " name="files" required> <br><br>
                                            <input style="border: none; cursor: pointer; border-radius: 10px; padding: 5px;" type="submit" value="Submit" name="submit-task">
                                        </form>
                                        <?php if($submit){ ?>
                                            <div class="alert">
                                                <p style="color: #d0fe1d; text-shadow: -1px 0 black, 0 1px black, 1px 0 black, 0 -1px black;
">Sudah Mengumpulkan</p>
                                            </div>
                                        <?php }else{?> 
                                            <div class="red-alert">
...
if(isset($_POST['submit-task'])){
    $idtugas = $id;
    $mahasiswaid = $_SESSION['a_global']->admin_id;
    ...
    if ($_FILES['files']['tmp_name']){
        ...
        $file_name = $submit->file_path;
    }

    $kirim = mysqli_query($conn,"INSERT INTO `tb_submit`(`submit_id`, `tugas_id`, `mahasiswa_id`, `submit_desc`, `submit_date`, `file_path`) VALUES ('','".$idtugas."','".$mahasiswaid."','".$deskripsi."','".$datesubmit."','".$file_name."') ");           
...
}else{ header('location: tugas.php'); }}else{
echo '<script>window.location="tugas.php"</script>';
}?>
